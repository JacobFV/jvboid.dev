...
date: 2025-11-23
resume_description: "python implementation of the EGGROLL low-rank evolution-strategies algorithm for training billion-parameter models without gradients. python, pytorch, evolution strategies."
lane: building
tags: [personal]
summary: "Python implementation of EGGROLL (Evolution Guided General Optimization via Low-rank Learning) for scalable evolution strategies training"
hero:
  src: /assets/img/projects/eggroll-trainer.svg
  alt: "eggroll-trainer icon"
links: 
  github: https://github.com/JacobFV/eggroll-trainer
---
...
- <strong>Initialization:</strong> Initialize base parameters $\theta_0$

- <strong>For each generation $t$:</strong>



- For each population member $i \in \{1, \ldots, N\}$:



- For each matrix layer $W \in \mathbb{R}^{m \times n}$:



- Sample $A_i \in \mathbb{R}^{m \times r}$, $B_i \in \mathbb{R}^{n \times r}$ from $\mathcal{N}(0, I)$

- Form low-rank perturbation: $\Delta W_i = A_i B_i^\top$

- Compute perturbed parameters: $\theta_i = \theta_t + \sigma \cdot \Delta_i$



- Evaluate fitness: $f_i = F(\theta_i)$



- <strong>Update:</strong> $\theta_{t+1} = \theta_t + \alpha \cdot \frac{1}{N\sigma} \sum_{i=1}^N f_i \cdot \Delta_i$
...
- <strong>Memory:</strong> $O(r(m+n))$ per layer vs. $O(mn)$ for full-rank ES

- <strong>Computation:</strong> $O(r(m+n))$ per forward pass vs. $O(mn)$ for full-rank ES

- <strong>Convergence:</strong> Low-rank updates converge to full-rank at rate $O(1/r)$
...
- Maintains high-rank updates through population averaging despite low-rank perturbations

- Scales linearly with rank $r$ rather than quadratically with layer dimensions

- Enables efficient batched computation compatible with modern GPU architectures

- No gradient computation required—pure black-box optimization
