...
The distinction matters. SynthUX now records low-level cursor and keyboard evidence, and the visible transition is caused by replayed input. But the web desktop is still hosting a dataset workbench rather than each terminal affordance being expressed through native simulator app internals. That collapses the visible affordance space: a row may contain GitHub, editor, terminal, Slack, browser, Notion, and dashboard terminals, while the video still mostly shows one workbench application.

That is the main gap in the current dataset. The pseudo-context-sensitive grammar exists to explode choices across application, action, target, payload, order, branch, and cross-app handoff dimensions. Future runs need app-specific low-level compilers so those choices are visible as movement across simulator applications, not just represented as labels in one workbench. The schema is meant to survive that upgrade: the target side remains the tree, the observed side remains the trajectory, and native app semantics become a better execution substrate rather than a new dataset format.
