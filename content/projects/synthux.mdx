...
| Surface   | browser-os         | windows-web-next | macos-web-next |
|-----------|--------------------|------------------|----------------|
| github    | browser            | edge             | safari         |
| browser   | browser            | edge             | safari         |
| dashboard | browser (Grafana)  | edge             | safari         |
| editor    | notepad            | vscode           | vscode         |
| terminal  | terminal           | terminal         | terminal       |
| notion    | notes              | wordpad          | notes          |
| slack     | messaging-client   | mail             | messages       |
...
The single-workbench visual-collapse failure mode is closed architecturally — `synthux validate` flags any row that fell back to the workbench overlay for all observations, and the build summary now publishes an `app_native_ratio` per dataset. Two known gaps remain:

- The ~19% of observations that still fall back are concentrated in browser-os Slack and Notes. The messaging-client app needs a pre-existing conversation before its message input appears, and the Notes app's "New Note" affordance is fragile to layout shifts. Adding stable test ids to those two apps would push browser-os to 100% app-native and is the highest-leverage simulator-side change left.
- The dataset published here is intentionally modest (9 rows) to bound disk and iteration time during the architectural upgrade. With frame-capture throttling in place, larger length-multiplied runs are now safe.

The schema survives both improvements: the target side remains the tree, the observed side remains the trajectory, and the per-(env, surface) compiler dispatch is the substrate that gets richer over time. The 2026-05-24 upgrade replaced the single-workbench codepath; the next upgrades are pure compiler/selector work and per-app shim polish inside the simulators.
